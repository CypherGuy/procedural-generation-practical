...
# The formula we're using here is called the Mixed Congruential Generator (MCG),
# the backbone of all the rest of the code. This will form the pseudorandomness of the maps
...
""" With a mixed LCG, there's rules to adhere to in order to get the maximum period
(Most numbers without repeating a cycle):
...
# We call the generate states function and see if it should be a wall. Note that we should avoid
# modding by 2,4 and 8 as those are powers of two and with Mixed LCG's, this may not be random
...
def generate_map(local_seed, map_size):
    ...
    # I realised at some point in testing that buildings always end up the same size and the entrance
    # in the same direction no matter the wall size. in order to fix this, I create new seeds based
    # off of the intial seed for all major obstacles.
    wall_seed = (local_seed * 6364136223846793005 +
                 1442695040888963407) & 0xFFFFFFFF
    building_seed = (local_seed * 1103515245 + 12345 +
                     map_size * 2654435761) & 0xFFFFFFFF
    lake_seed = (local_seed * 24597843 + 13497 +
                 map_size * 3592078431) & 0xFFFFFFFF
    ...
    def get_all_valid_building_spots(size):
        ...
        for i in range(1, map_size - size):
            for j in range(1, map_size - size):
                if grid[i][j] == "g":
                    # To stop buildings from spawning on top of each other, we can check
                    # if the entire proposed building is grass
                    clear = True
    ...
    # To deterministically pick player, building and treasure spots, we can simply do something like
    # seed % (total of grass places) and then use that to decide where to put the player/treasure.
    ...
    for i in range(min(8, map_size // 20)):
        # To stop the same building shape being generated multiple times,
        # we call the next state multiple times i+1 times each iteration
        for _ in range(i+1):
            current_state = next(state_gen)
        grass_spots = sum(row.count("g") for row in grid)
        # If 0 -> 4x4, if 1 -> 5x5, if 2 -> 6x6
        building_size = (current_state % 3) + 4
        ...
        if not valid_building_spots:
            continue
        bx, by = valid_building_spots[current_state %
                                      len(valid_building_spots)]
        placed_buildings.append((bx, by))

        # Now we build the building. To work out how many total building spots have been placed,
        # we can use the nth term an = 4n-4. But to exclude the corners, we subtract 4 more, so 8 total.
        ...
        for i in range(building_size):
            for j in range(building_size):
                # One thing I noticed during testing is that walls can sometimes block access to,
                # or be inside, the building. As such I'll replace all walls in the building with
                # grass, then switch the block outside the entrance to grass
                if grid[bx+i][by+j] == "w":
                    grid[bx+i][by+j] = "g"
                if i == 0 or i == building_size - 1 or j == 0 or j == building_size - 1:
                    ...
                    is_corner = (
                        (i == 0 and j == 0)
                        or (i == 0 and j == building_size - 1)
                        or (i == building_size - 1 and j == 0)
                        or (i == building_size - 1 and j == building_size - 1)
                    )
                    if not is_corner:

                        if count == building_entrance:
                            grid[bx+i][by+j] = "g"
                            # Prevent walls blocking the entrance from outside: clear the tile
                            # on whichever edge the entrance sits on
                            if i == 0:
                                grid[bx+i-1][by+j] = "g"
    ...
    New: We want lakes to not just be lines but to actually close on each other,
    forming a circle or some other shape. Our strategy is to generate the lake as normal,
    find the two furthest points from each other and connect them.
    """
    ...
    def count_lake_spots():
        ...
        for i in range(gap, map_size-gap):
            for j in range(gap, map_size-gap):
                clear_row = all(grid[i+k][j] not in ("b", "l") for k in range(-gap, gap+1))
                clear_col = all(grid[i][j+k] not in ("b", "l") for k in range(-gap, gap+1))
                if grid[i][j] == "g" and clear_row and clear_col:
    ...
    for i in range(min(4, map_size // 40)):
        ...
        amount_of_spots = ((next(lake_state_gen) % 30) +
                           20) - 1  # 20 to 50 blocks total
        frontier = [lake_pos]
        ...
        # Lake line drawn. Now collect all lake positions to find the endpoints.
        lake_visited = []
        ...
        # Pick a random point, find the furthest from it, then the furthest from that one
        random_point = lake_visited[(next(lake_state_gen) % len(lake_visited))]
        furthest_point = max(
            lake_visited,
            key=lambda point: ((point[0]-random_point[0])**2 + (point[1]-random_point[1])**2)**0.5
        )
        other_furthest_point = max(
            lake_visited,
            key=lambda point: ((point[0]-furthest_point[0])**2 + (point[1]-furthest_point[1])**2)**0.5
        )

        # Close the lake into a loop if the two endpoints are close enough relative to total length.
        # The higher the threshold, the more often a river becomes a closed lake.
        total_lake_length = len(lake_visited)
        endpoint_dist = (
            (other_furthest_point[0]-furthest_point[0])**2
            + (other_furthest_point[1]-furthest_point[1])**2
        )**0.5
        if total_lake_length * 0.65 > endpoint_dist:

            def draw_pixel(from_coordinate, to_coordinate, neighbour):
                """Fill neighbours probabilistically, favouring tiles closer to the target."""
            ...
            # Smooth out square edges: any grass tile with 3+ lake neighbours becomes lake.
            # Operate on a deep copy so mid-pass placements don't affect the same pass.
            for _ in range(4):
...
def on_regenerate_button_pressed(seed, map_size, grid):
    ...
    # On an invalid world, relocate the player and treasure — that's enough for most cases.
    # Mix the seed so consecutive regenerate clicks (seed+1, seed+2, ...) produce
    # vastly different starting states rather than outputs only 5 apart.
    mixed_seed = (seed * 6364136223846793005 + 1442695040888963407) & 0xFFFFFFFF
    state_gen = generate_states(mixed_seed, map_size)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_UP:
                    ...
                    seed_changed = True
                    next_repeat_ms[event.key] = pygame.time.get_ticks(
                    ) + key_repeat_delay_ms
                ...
                if not is_possible and regenerate_button_rect is not None:
                    if regenerate_button_rect.collidepoint(event.pos):
                        ...
                        regenerate_count += 1
                        result = on_regenerate_button_pressed(
                            current_seed + regenerate_count, current_size, grid)
        ...
        for key, delta in size_hold_actions.items():
            if pressed[key] and key in next_repeat_ms and now >= next_repeat_ms[key]:
                ...
                current_size = max(min_map_size, min(
                    max_map_size, current_size + delta))
        ...
        if pressed[pygame.K_BACKSPACE] and pygame.K_BACKSPACE in next_repeat_ms:
            if now >= next_repeat_ms[pygame.K_BACKSPACE]:
                if seed_input:
                    seed_input = seed_input[:-1]
                next_repeat_ms[pygame.K_BACKSPACE] = now + \
        ...
        if not is_possible:
            ...
            button_y = 84
            regenerate_button_rect = pygame.Rect(
                button_x, button_y, button_w, button_h)
            pygame.draw.rect(screen, (60, 60, 60),
                             regenerate_button_rect, border_radius=6)
            pygame.draw.rect(screen, (210, 210, 210),
                             regenerate_button_rect, width=1, border_radius=6)
            button_text = font.render(
                "Regenerate World", True, (240, 240, 240))
            text_rect = button_text.get_rect(
                center=regenerate_button_rect.center)
        ...
        if error_bubble_text and pygame.time.get_ticks() < error_bubble_until_ms:
            ...
            pygame.draw.rect(screen, (120, 20, 20),
                             bubble_rect, border_radius=8)
            pygame.draw.rect(screen, (220, 130, 130),
                             bubble_rect, width=1, border_radius=8)
        ...
        if action_bubble_text and pygame.time.get_ticks() < action_bubble_until_ms:
            ...
            bubble_text = font.render(
                action_bubble_text, True, (235, 235, 245))
            bubble_w = bubble_text.get_width() + bubble_pad_x * 2
            bubble_x = max(8, (screen.get_width() - bubble_w) // 2)
            bubble_y = hud_height - 70
            bubble_rect = pygame.Rect(bubble_x, bubble_y, bubble_w, bubble_h)
            pygame.draw.rect(screen, action_bubble_color,
                             bubble_rect, border_radius=8)
            pygame.draw.rect(screen, (210, 210, 210),
                             bubble_rect, width=1, border_radius=8)
        ...
        tile_size = max(1, min(screen_w // current_size,
                        grid_area_h // current_size))
        ...
        for i in range(current_size):
            for j in range(current_size):
                ...
                rect = (origin_x + j * tile_size, origin_y +
